$('document').ready(function () {
    ...
    function checkSort(){               //decalre a function to check selected sort
    ...
    $('#search').click(function(){
        ...
        searchTerm = searchTerm.trim().toUpperCase().split(' ');
        for(i=0; i<=searchTerm.length-1; i++){  //for every word in the user's input
        ...
        else{ //if begind date is selected/input
        ...
        $.ajax ({
            ...
            type: 'GET',
            url: 'https://lgapi-us.libapps.com/1.1/guides/?site_id=8488&key=0b8da796b00334ae3471f60e6a10e8c6'+ sortvalue + '&sort_dir=asc&expand=owner,',
            success: function(guides) {
                $.each(guides, function(i, guide) { //for every guide in the request
                    ...
                    for(i=0; i<nameArr.length; i++){ //for every word in the name
                    ...
                    for(i=0; i<groupArr.length; i++){   //for every word in the group type
                    ...
                    if ((status != 0) && (status != 2)){    //if the guide is neither private or not published (aka is published)
                        // check to see if the user's search word(s) are included in various guide attributes
                        if( (fullname.includes(searchArr)) ||  (title.includes(searchArr)) || (group_label.includes(searchArr)) || (desc.includes(searchArr)) || (fname.includes(searchTerm)) || (lname.includes(searchTerm)) || (id.includes(searchTerm))  ){ 
                            if((startdate <= pubdate) && (pubdate <= finaldate)){ //if the guide was published with in the dates the user is looking at
                                //output a card with the guide's information and a link to the guide
                                $output.append( '<div class="card border-info mb-3">' + 
                                    '<h4 class="card-header">' + guide.name + ' </h4> <ul> '
                                    + '<li> Author: ' + guide.owner.first_name + ' ' + guide.owner.last_name + '</li>'
                                    + '<li> Description: ' + guide.description + '</li>' 
                                    + '<li> Type: ' + guide.type_label + '</li>'
                                    + '<li> Published on: ' + date + '</li>'
                                    + '<li> <a href="' + guide.url + '">' + 'Read More' + '</a> </li> </div>');
                ...
                if (noresults == false){ //if there were results
                    $numres.append('<h6> Showing ' + count + ' results.</h6>');     //output the number of results
                }
                
                if(noresults == true){  //if there were no results
                    $output.append("No results."); //say no results
                }
            },
    
            error: function(){  //if there was an error loading
